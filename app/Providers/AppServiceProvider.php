        ...
        \Illuminate\Support\Facades\View::composer('partials.header', function (\Illuminate\View\View $view) {
            ...
            if (in_array($role, ['admin', 'owner'], true)) {
                $counts = \Illuminate\Support\Facades\Cache::remember(
                    ...
                    now()->addMinutes(5),
                    function () {
                        $summary = \App\Models\Ingredient::query()
                            ->selectRaw(
                                'SUM(CASE WHEN stock <= 0 THEN 1 ELSE 0 END) as out_count,
                                SUM(CASE WHEN stock > 0 AND stock <= minimum_stock THEN 1 ELSE 0 END) as low_count'
                            )
                            ->first();

                        return [
                            'outOfStock' => (int) ($summary->out_count ?? 0),
                            'lowStock' => (int) ($summary->low_count ?? 0),
                        ];
                    }
                );
