...
use App\Exports\QueuedUsageReportExport;
...
class GenerateUsageReportExport implements ShouldQueue
{
    ...
    public function handle(UsageReportExportQuery $queryService, ?GeneratedExportLifecycle $lifecycle = null): void
    {
        $lifecycle ??= app(GeneratedExportLifecycle::class);
        $export = GeneratedExport::query()->find($this->generatedExportId);
        if ($export === null || $export->type !== 'usage_report' || $export->format !== 'excel' || ! $lifecycle->claim($export->id, $this->attempts())) return;
        $export->refresh();
        $disk = 'local';
        $path = null;
        try {
            $filters = $export->filters ?? [];
            $query = $queryService->build(Carbon::parse($filters['date_from'])->startOfDay(), Carbon::parse($filters['date_to'])->endOfDay(), $export->branch_id);
            $path = 'exports/' . $export->requested_by . '/' . $export->id . '/' . $export->original_filename;
            Storage::disk($disk)->delete($path); Excel::store(new QueuedUsageReportExport($query), $path, $disk);
