...
<div class="w-full space-y-6 overflow-x-hidden pb-10">
    ...
    <x-page-header 
        title="Varian Menu - {{ $menu->name }}" 
        subtitle="Kelola varian dari menu {{ $menu->name }}. Setiap varian memiliki harga modal (HPP) dan harga jual yang berbeda-beda." 
        breadcrumb-parent="Manajemen Menu" 
        breadcrumb-child="Varian Menu">
                        ...
                        <tr class="hidden md:table-row hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group">
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex items-center justify-end gap-2.5">
                                    <a href="{{ route('admin.menu-variants.edit', [$menu->id, $variant->id]) }}" class="inline-flex items-center justify-center rounded-xl bg-blue-50 p-2 text-blue-700 transition hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-400 dark:hover:bg-blue-500/20 shadow-sm" title="Edit Varian">
                                    ...
                                    <form action="{{ route('admin.menu-variants.destroy', [$menu->id, $variant->id]) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus varian menu ini?')" class="inline-flex items-center justify-center rounded-xl bg-rose-50 p-2 text-rose-700 transition hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:hover:bg-rose-500/20 shadow-sm" title="Hapus Varian">
                        ...
                        <tr class="md:hidden">
                            <td colspan="6" class="p-0">
                                <div class="p-5 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                    ...
                                    <div class="flex items-center justify-end gap-2.5 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                                        <a href="{{ route('admin.menu-variants.edit', [$menu->id, $variant->id]) }}" class="inline-flex items-center justify-center text-blue-700 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors bg-blue-50 dark:bg-blue-500/10 p-2 rounded-xl shadow-sm" title="Edit Varian">
                                        ...
                                        <form action="{{ route('admin.menu-variants.destroy', [$menu->id, $variant->id]) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus varian menu ini?')" class="inline-flex items-center justify-center text-rose-700 hover:text-rose-800 dark:text-rose-400 dark:hover:text-rose-300 transition-colors bg-rose-50 dark:bg-rose-500/10 p-2 rounded-xl shadow-sm" title="Hapus Varian">
