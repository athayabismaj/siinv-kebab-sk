...
<div class="space-y-8 max-w-full overflow-x-hidden">
    <div class="mb-8">
        ...
        <h1 class="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-3">Laporan Pemakaian</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-5">
            Pantau total bahan baku yang terpakai berdasarkan periode harian, mingguan, atau bulanan.
        </p>
    ...
    <form method="GET" action="{{ route($usageRoute) }}" id="filter-form" class="flex flex-col lg:flex-row gap-3 w-full mb-6 relative z-10">
        ...
        <div class="flex flex-col sm:flex-row gap-3 flex-1">
            ...
            <div class="flex-1 flex items-center px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all min-w-0">
                ...
                <input type="text" name="search" id="search-input" value="{{ $search }}" placeholder="Cari nama bahan baku..." autocomplete="off" class="w-full bg-transparent border-none text-[13px] font-semibold text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-0 p-0 outline-none">
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div class="relative p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-3">Item Terpakai</p>
            <p class="text-3xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">{{ number_format($summary['ingredients_count']) }} <span class="text-sm text-slate-400 font-bold normal-case ml-1">Bahan</span></p>
        </div>

        <div class="relative p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-3">Frekuensi Pemakaian</p>
            <p class="text-3xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">{{ number_format($summary['logs_count']) }} <span class="text-sm text-slate-400 font-bold normal-case ml-1">Log</span></p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm self-start">
        <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Rincian Pemakaian Bahan</p>
            <p class="text-[10px] font-bold text-slate-400">Menampilkan <span class="text-slate-700 dark:text-slate-300">{{ $usageItems->firstItem() ?? 0 }} - {{ $usageItems->lastItem() ?? 0 }}</span> dari <span class="text-slate-700 dark:text-slate-300">{{ $usageItems->total() }}</span> data</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($usageItems as $item)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                            <td class="px-6 py-4 font-bold text-slate-800 dark:text-white">{{ $item->ingredient_name }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-3 py-1.5 bg-red-50 dark:bg-red-900/30 border border-red-100 dark:border-red-800/50 rounded-lg font-black text-red-600 dark:text-red-400 text-[13px] tracking-tight">{{ $formatted }} <span class="text-[10px] font-bold uppercase tracking-widest ml-0.5">{{ $displayUnit }}</span></span>
                            </td>
                            <td class="px-6 py-4 text-center"><span class="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-md font-bold text-slate-600 dark:text-slate-300 text-xs">{{ number_format($item->usage_count) }}x</span></td>
                            <td class="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 tabular-nums">{{ \Carbon\Carbon::parse($item->last_used_at)->translatedFormat('d M Y, H:i') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-20 text-center text-slate-400 dark:text-slate-500 text-sm font-medium">Tidak ada pemakaian bahan dalam rentang tanggal ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($usageItems->hasPages())
            <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex justify-end items-center">
