<form method="GET" action="{{ route($usageRoute) }}" id="filter-form" class="relative z-10 py-2">
    ...
    <div class="flex flex-col lg:flex-row gap-3 w-full items-center justify-between">
        ...
        <div class="flex items-center w-full lg:w-auto shrink-0 justify-end">
            <a href="{{ route($exportRoute, request()->query()) }}" class="flex-1 lg:flex-none inline-flex items-center justify-center gap-2 px-5 h-[38px] bg-slate-900 text-white text-[13px] font-semibold rounded-xl hover:bg-slate-800 transition-all shadow-sm dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">
