<form method="GET" action="{{ route($usageRoute) }}" id="filter-form" data-period-filter class="relative z-10 py-2">
    ...
    <div class="flex flex-col lg:flex-row gap-3 w-full items-stretch lg:items-center">
        ...
        <div class="flex flex-row gap-3 shrink-0">
            ...
            {{-- Atur Ulang Button --}}
            @if(request()->filled('date_from') || (request()->filled('type') && request('type') !== 'daily'))
                <a href="{{ route($usageRoute) }}" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 h-[38px] text-[13px] font-semibold text-slate-600 shadow-sm transition-all hover:bg-slate-50 hover:text-rose-600 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-rose-400">
