<form method="GET" action="{{ route('admin.menus.archive') }}" class="flex flex-col sm:flex-row gap-3 w-full relative z-10 py-2 mb-2">
    <div class="flex-1 relative flex items-center w-full rounded-xl border border-slate-200 bg-white shadow-sm transition-all focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 dark:border-slate-800 dark:bg-slate-900">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama menu nonaktif..."
    ...
    <div class="flex items-center gap-3 shrink-0 justify-end w-full sm:w-auto">
        @if(request()->filled('search') || request()->filled('category'))
            <a href="{{ route('admin.menus.archive') }}" class="mr-1 inline-flex items-center gap-1.5 text-[12px] font-semibold text-slate-400 hover:text-red-500 transition-colors">
